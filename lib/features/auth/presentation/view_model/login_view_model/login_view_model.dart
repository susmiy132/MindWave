...
class LoginViewModel extends Bloc<LoginEvent, LoginState> {
  ...
  // ✅ Injected snackbar callback for testability
  final void Function(BuildContext, String)? showError;

  LoginViewModel(this._userLoginUsecase, {this.showError})
  ...
  void _onLoginWithEmailAndPassword(
    ...
    result.fold(
      (failure) {
        ...
        // ✅ Use injected function instead of calling showMySnackBar directly
        showError?.call(event.context, 'Invalid credentials. Please try again.');
      },
      (token) {
        emit(state.copyWith(isLoading: false, isSuccess: true));
        add(NavigatetoHomeViewEvent(context: event.context));
